...
describe("Context overflow error handling", () => {
	...
	describe.skipIf(!process.env.GEMINI_API_KEY)("Google", () => {
		it("Google model - should detect overflow via isContextOverflow", async () => {
			const model = getModels("google").find(
				(candidate) => candidate.id === "gemini-2.5-flash" || candidate.id.startsWith("gemini-"),
			);
			if (!model) throw new Error("No Google Gemini models available");
